export default [
    {
        id: 1,
        title: "Application web en RedwoodJS",
        description: "Création d'une application web de la seconde main avec RedwoodJS, utilisant Prisma, GraphQL et React. (en cours de développement)",
        longDescription: "Cette plateforme innovante de seconde main vise à révolutionner la façon dont les utilisateurs recherche des produits d'occasion. En utilisant l'architecture full-stack de RedwoodJS, nous avons créé une expérience utilisateur fluide tout en maintenant une base de données robuste via Prisma. Le projet implémente des fonctionnalités avancées de recherche et de filtrage pour aider les utilisateurs à trouver exactement ce qu'ils cherchent.",
        image: "/principal_jaune.png",
        tags: ["RedwoodJS", "React", "Prisma", "GraphQL", "TypeScript", "Tailwind CSS"],
        link: "",
        github: "",
        features: [],
        technologies: [
            "RedwoodJS", "React", "Prisma", "GraphQL", "PostgreSQL", "TypeScript", "Tailwind CSS", "Jest"
        ],
        challenges: [],
        outcomes: []
    },
    {
        id: 2,
        title: "Encrypt",
        description: "Site web qui permet de crypter un texte et d'y mettre un mot de passe pour le décrypter.",
        longDescription: "Encrypt est une application web de chiffrement de bout en bout qui permet aux utilisateurs de sécuriser leurs messages sensibles. Utilisant des algorithmes de cryptographie modernes (AES-256), l'application génère des messages chiffrés qui ne peuvent être déchiffrés qu'avec le mot de passe correct. Aucune donnée n'est stockée sur le serveur, garantissant ainsi une confidentialité totale et un respect de la vie privée des utilisateurs.",
        image: "/x06zx7JRRPKIpzSPfYXafA.webp?height=600&width=800",
        tags: ["HTML/CSS", "JavaScript", "Cryptographie", "Sécurité"],
        link: "https://guillaume-breon.tech/Encrypt/",
        github: "",
        features: [
            "Cryptage AES-256 avec dérivation de clé sécurisée",
            "Interface minimaliste et intuitive",
            "Génération de liens partageables sécurisés",
        ],
        technologies: [
            "JavaScript vanilla", "HTML5", "Tailwind", "Responsive Design"
        ],
        outcomes: [
            "Adoption par plusieurs entreprises pour la communication de données sensibles",
            "Temps de chiffrement moyen inférieur à 500ms même pour les longs messages"
        ]
    },
    {
        id: 3,
        title: "Pokedex",
        description: "Création d'un Pokedex en React en utilisant une API privé pour récupérer les informations des pokémons. (en cours de développement)",
        longDescription: "Ce Pokedex interactif offre une encyclopédie complète des Pokémon avec une interface moderne et responsive. En exploitant une API propriétaire, l'application fournit des données détaillées sur chaque créature, incluant statistiques, évolutions, attaques et habitats. Les utilisateurs peuvent rechercher par nom, type ou région, et comparer les caractéristiques de différents Pokémon. Le design s'inspire des jeux originaux tout en apportant une touche contemporaine.",
        image: "/image.png?height=600&width=800",
        tags: ["React", "Axios", "TypeScript"],
        link: "",
        github: "",
        features: [
            "Base de données complète des 898 Pokémon",
            "Filtrage avancé par type, génération et caractéristiques",
            "Système de favoris personnalisable",
            "Visualisation des chaînes d'évolution",
            "Mode combat simulé pour comparer les statistiques"
        ],
        technologies: [
            "React", "Axios", "TypeScript"
        ],
    },
    {
        id: 4,
        title: "Jeu en Réalité Virtuelle",
        description: "Jeu en réalité virtuelle et 3ème personne réalisé lors d'un workshop inter-campus utilisant Unreal Engine 5.",
        longDescription: "Cette expérience immersive en réalité virtuelle plonge les joueurs dans un monde post-apocalyptique où ils doivent résoudre des énigmes environnementales pour survivre. Développé en 72 heures lors d'un workshop inter-campus intensif, le jeu exploite les capacités graphiques avancées d'Unreal Engine 5 avec le système Nanite pour des environnements ultra-détaillés et Lumen pour un éclairage dynamique photoréaliste. L'interaction physique avec l'environnement est au cœur du gameplay.",
        image: "/aAMV7s5uQqeDJaA69z1R5A.webp?height=600&width=800",
        tags: ["Unreal Engine 5", "VR", "BluePrint", "3D Modeling", "Game Design"],
        link: "https://youtu.be/T_Tb4zZbmQo?si=TVGA59QA18LO2e2R",
        github: "",
        features: [
            "Environnements destructibles avec physique réaliste",
            "Support multi-plateforme (Oculus, Valve Index, HTC Vive)"
        ],
        technologies: [
            "Unreal Engine 5", "Blueprints", "Blender"
        ],
        challenges: [
            "Optimisation des performances pour maintenir 60 FPS constant sur matériel grand public",
            "Collaboration efficace entre 4 développeurs en temps limité",
            "Equilibrage entre qualité visuelle et fluidité de l'expérience VR"
        ],
        outcomes: [
            "Premier prix au concours inter-campus",
            "Présentation du projet lors de salon pour la présentation de l'école",
        ]
    },
    {
        id: 5,
        title: "Réseau Social en PHP",
        description: "Création d'un réseau social complet avec système d'authentification, profils utilisateurs et partage de contenu.",
        longDescription: "Cette plateforme sociale développée en PHP natif offre une alternative éthique aux géants du secteur en mettant l'accent sur la confidentialité des données. L'architecture MVC personnalisée garantit une séparation claire des responsabilités et facilite la maintenance. Les fonctionnalités incluent un système complet de gestion de contenu, des fils d'actualité personnalisés par algorithme et des groupes thématiques. La sécurité a été une priorité avec protection contre les injections SQL, XSS et CSRF.",
        image: "/php.png?height=600&width=800",
        tags: ["PHP", "MySQL"],
        link: "",
        github: "",
        features: [],
        technologies: [
            "PHP", "MySQL"
        ]
    },
    {
        id: 6,
        title: "Site web pour une entreprise de BTP",
        description: "Création d'un site web pour une entreprise de BTP pour présenter ses différents projets et services.",
        longDescription: "Ce site vitrine pour CATS-BTP représente une refonte complète de leur présence en ligne, mettant en valeur leur expertise dans le secteur du BTP. L'accent a été mis sur l'expérience utilisateur avec une navigation intuitive et un design responsive qui s'adapte à tous les appareils. Le site intègre un portfolio interactif des projets réalisés avec galeries photos avant/après, un système de devis en ligne, et une carte interactive des chantiers. Les performances et le référencement ont été optimisés pour maximiser la visibilité de l'entreprise.",
        image: "/cropped-logo-base-cats-texte-horizontal-180x180.jpg?height=600&width=800",
        tags: ["HTML/CSS", "JavaScript", "Responsive Design", "SEO"],
        link: "https://www.cats-btp.fr/",
        github: "",
        features: [
        ],
        technologies: [
            "HTML5", "Tailwind"
        ],
        challenges: [
        ],
        outcomes: [
            "Augmentation de 30% du trafic organique en 6 mois",]
    },
    {
        id: 7,
        title: "Application mobile en React Native",
        description: "Création d'une application mobile en React Native type google keep. (en cours de développement)",
        longDescription: "Cette application de prise de notes inspirée par Google Keep combine simplicité d'utilisation et fonctionnalités avancées. Développée avec React Native pour une expérience native sur iOS et Android, elle se synchronise en temps réel via Firebase et fonctionne également hors ligne grâce à un système de mise en cache intelligent. L'application propose des fonctionnalités d'organisation avancées comme les tags, le code couleur, les rappels géolocalisés et la reconnaissance vocale pour la création rapide de notes.",
        image: "/975TcgONQd6XMHJYVNgZDw.jpeg?height=600&width=800",
        tags: ["React Native", "TypeScript", "Expo"],
        link: "",
        github: "",
        features: [
            "Mode sombre et personnalisation de l'interface",
            "Organisation par tags, couleurs et collections"
        ],
        technologies: [
            "React Native", "Expo", "TypeScript"
        ],
        outcomes: []
    }
]